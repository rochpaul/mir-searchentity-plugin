            ...
            $(data).each(function (index, item) {
                ...
                if (that.selectedType === "GND" && options.target === ".personExtended_box") {
                    $person.on("mouseover", function (event) {
                        ...
                        let lobidBaseUrl = 'https://lobid.org/';
                        let lobidUrl = item.value.replace("http://d-nb.info", lobidBaseUrl);

                        SearchEntity.loadData(lobidUrl, 'json', '', (entityDetails) => {

                            var parseMultipleDetail = (parsedDetails, detailIdenfitier, entityDetail) => {
                                parsedDetails[detailIdenfitier] = '';
                                entityDetail.forEach((val, key, arr) => {
                                    if (val.label) {
                                        val = val.label;
                                    }

                                    parsedDetails[detailIdenfitier] = parsedDetails[detailIdenfitier] +
                                        val;
                                    if (key !== arr.length - 1) {
                                        parsedDetails[detailIdenfitier] = parsedDetails[detailIdenfitier]
                                            + ', ';
                            ...
                            /* gender */
                            if (entityDetails.gender && entityDetails.gender[0].label
                                && entityDetails.gender[0].label !== 'Unbekannt') {
                                parsedDetails["gender"] = entityDetails.gender[0].label;
                            }
                            /* date of birth */
                            if (entityDetails.dateOfBirth && entityDetails.dateOfBirth[0] !== '[XXXX]') {
                                parsedDetails["dateOfBirth"] = entityDetails.dateOfBirth[0];
                            }
                            /* place of birth */
                            if (entityDetails.placeOfBirth && entityDetails.placeOfBirth[0].label) {
                                parsedDetails["placeOfBirth"] = entityDetails.placeOfBirth[0].label;
                            }
                            /* academic degree */
                            if (entityDetails.academicDegree) {
                                parsedDetails["academicDegree"] = entityDetails.academicDegree[0];
                            }
                            /* profession (multiple)*/
                            if (entityDetails.professionOrOccupation) {
                                parseMultipleDetail(parsedDetails, "professionOrOccupation",
                                    entityDetails.professionOrOccupation);
                            }
                            /* geographicalAreaCodes (multiple)*/
                            if (entityDetails.geographicAreaCode) {
                                parseMultipleDetail(parsedDetails, "geographicAreaCode",
                                    entityDetails.geographicAreaCode);
                            }
                            /* placeOfActivity (multiple) */
                            if (entityDetails.placeOfActivity) {
                                parseMultipleDetail(parsedDetails, "placeOfActivity",
                                    entityDetails.placeOfActivity);
                            }
                            /* periodOfActivity (multiple) */
                            if (entityDetails.periodOfActivity) {
                                parseMultipleDetail(parsedDetails, "periodOfActivity",
                                    entityDetails.periodOfActivity);
                            }
                            /* affiliations (multiple) */
                            if (entityDetails.affiliation) {
                                parseMultipleDetail(parsedDetails, "affiliation",
                                    entityDetails.affiliation);
                            }
                            /* publication (multiple) */
                            if (entityDetails.publication) {
                                parseMultipleDetail(parsedDetails, "publication",
                                    entityDetails.publication);
                            }
                            /* biographicalOrHistoricalInformation (multiple) */
                            if (entityDetails.biographicalOrHistoricalInformation) {
                                parseMultipleDetail(parsedDetails, "biographicalOrHistoricalInformation",
                                    entityDetails.biographicalOrHistoricalInformation);
                            }

                            console.log(parsedDetails);

                            // for (const key of Object.keys(parsedDetails)) {
                            //     console.log(key, parsedDetails[key]);
                            // }
                            ...
                            console.log('Error on fetching further gnd information with url: ' + lobidUrl);
                        })
                    });
                }
